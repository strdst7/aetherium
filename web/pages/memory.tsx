...
export default function MemoryViz() {
  ...
  const handleTrace = async (e: React.FormEvent) => {
    ...
    setTracing(true);
    try {
      const res = await fetch(`http://localhost:8080/v1/memory/trace?query=${encodeURIComponent(query)}`);
  ...
  useEffect(() => {
    async function load() {
      try {
        const res = await fetch("http://localhost:8080/v1/memory/all");
  ...
  if (loading) return <div style={{ padding: 40 }}>Loading identity map...</div>;
