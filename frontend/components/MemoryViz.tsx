...
import { Search, Database, Fingerprint, Zap } from 'lucide-react';
...
const CustomTooltip = ({ active, payload }: any) => {
  if (active && payload && payload.length) {
    ...
    return (
      <div className="bg-aether-900 border border-aether-orange p-3 rounded-lg text-xs font-mono text-slate-200 shadow-[0_0_15px_rgba(255,122,0,0.2)] z-50">
  ...
  return (
    <div className="p-8 h-full flex flex-col">
      <header className="mb-6 flex justify-between items-end">
        ...
        <div className="flex items-center gap-3 bg-aether-800/80 border border-aether-orange/30 px-4 py-2 rounded-lg">
      ...
      <div className="flex gap-6 flex-1 min-h-0">
        ...
        <div className="w-1/3 flex flex-col gap-4">
          <form onSubmit={handleSearch} className="relative">
            <div className="relative flex items-center bg-aether-800 border border-aether-orange/30 rounded-lg p-2 focus-within:border-aether-orange transition-colors shadow-[0_0_10px_rgba(255,122,0,0.05)]">
          ...
          <div className="flex-1 bg-aether-800/50 border border-white/10 rounded-xl p-4 overflow-y-auto custom-scrollbar">
        ...
        <div className="flex-1 bg-aether-800/30 border border-white/10 rounded-xl p-4 flex flex-col relative overflow-hidden">
          {!useServerUmap && (
          ...
          <div className="flex-1 min-h-0">
            <ResponsiveContainer width="100%" height="100%">
              <ScatterChart margin={{ top: 20, right: 20, bottom: 20, left: 20 }}>
                ...
                <Scatter data={projectedMemories} fill="#ff007f" animationDuration={useServerUmap ? 500 : 0}>
                  {projectedMemories.map((entry, index) => {
                    const isResult = results.find(r => r.id === entry.id);
                    return <Cell 
                      key={`cell-${index}`} 
                      fill={isResult ? '#ff7a00' : entry.cluster === 'anomaly' ? '#e6005c' : '#4b5563'} 
